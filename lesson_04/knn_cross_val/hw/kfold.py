...
def kfold_cross_validation(X: np.ndarray, y: np.ndarray, k: int) -> List[Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]]:
    ...
    for i in range(k):
        start = i * fold_size
        end = (i + 1) * fold_size if i < k - 1 else n_samples  # last fold may be larger
...
def evaluate_accuracy(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    # Обчислюємо точність як відсоток правильних передбачень
...
def main() -> None:
    ...
    model = KNN(k=k)
    model.fit(X, y)  # Train on entire training data
    y_pred_test = model.predict(X_test)
    accuracy_test = evaluate_accuracy(y_test, y_pred_test)
    print(f"Test Accuracy: {round(accuracy_test, 2)}")

    avg_cv_accuracy = np.mean(accuracy_scores)
    print(f"Average Cross-Validation Accuracy: {round(avg_cv_accuracy, 2)}")
